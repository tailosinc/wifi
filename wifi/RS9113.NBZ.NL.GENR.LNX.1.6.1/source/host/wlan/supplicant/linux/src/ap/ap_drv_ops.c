		     ...
		     int center_segment0, int center_segment1)
{
	...
	/** Moved the below Code here, from the above func as I am facing Segmentation fault Issues, when Running AP mode. 
	 *  Need to Review it Sometime -- Rajashekar */
	...
	case VHT_CHANWIDTH_USE_HT:
		...
		break;
	case VHT_CHANWIDTH_80P80MHZ:
		if (!((hapd->iface->current_mode->vht_capab) & VHT_CAP_SUPP_CHAN_WIDTH_160_80PLUS80MHZ)) {
			wpa_printf(MSG_ERROR,
				   "80+80 channel width is not supported!");
			return -1;
		}
	...
	case VHT_CHANWIDTH_160MHZ:
		data.bandwidth = 160;
		if (!((hapd->iface->current_mode->vht_capab) & (VHT_CAP_SUPP_CHAN_WIDTH_160MHZ |
				  VHT_CAP_SUPP_CHAN_WIDTH_160_80PLUS80MHZ))) {
			wpa_printf(MSG_ERROR,
				   "160MHZ channel width is not supported!");
			return -1;
