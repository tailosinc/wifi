			       ...
			       struct wpa_ptk *ptk)
{
	...
	if (sm->key_mgmt == WPA_KEY_MGMT_CCKM) {
		struct cckm_ptk *cptk = (struct cckm_ptk *)ptk;
		/* Calculate MIC using KRK rather than WPA-802.1X key */
		wpa_printf(MSG_INFO, "Key mgmt is CCKM, calculate MIC using KRK");
		wpa_eapol_key_send(sm, cptk->krk, ver, dst, ETH_P_EAPOL,
					  ...
					  u16 ver)
{
	...
	if (wpa_sm_get_network_ctx(sm) == NULL) {
 	...
 	if(WPA_COMPLETED == wpa_sm_get_state(sm)){
     		eapol_drp = 1;
	 	// wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: WPA COMPLETED. DROPPING 1/4!");
     		return;
   	}
				      ...
				      const struct wpa_eapol_key *key)
{
	...
	if(sm->key_mgmt == WPA_KEY_MGMT_CCKM)
	{
 	...
 	/* TK is not needed anymore in supplicant:FIXME */
 	os_memset(sm->ptk.tk1, 0, 16);
 	os_memset(sm->ptk.u.tk2, 0, 16);
 	sm->ptk.installed = 1;
			       ...
			       struct wpa_ptk *ptk)
{
	...
	{
		struct cckm_ptk *cptk = (struct cckm_ptk *)ptk;
		wpa_eapol_key_send(sm, cptk->kck, ver, dst, ETH_P_EAPOL,
				rbuf, rlen, reply->key_mic);
	}
	else
#endif
...
#ifdef RSI_CCX
	if (sm->key_mgmt == WPA_KEY_MGMT_CCKM) {
		wpa_eapol_key_send(sm, sm->cptk.kck, ver, sm->bssid, ETH_P_EAPOL,
				rbuf, rlen, reply->key_mic);
	}else
#endif 
	{
				       ...
				       u8 *gd)
{
	...
	wpa_eapol_key_mic(sm->cptk.kck, ver, buff, length, mic_new);
					       ...
					       const u8 *buf, size_t len)
{
	...
	if (sm->tptk_set) {
		...
		wpa_eapol_key_mic(kck, ver, buf, len,
				key->key_mic);
		    ...
		    const u8 *buf, size_t len)
{
	...
	if (key_info & WPA_KEY_INFO_KEY_TYPE) {
		...
		if (peerkey) {
			...
			if(eapol_drp){
				wpa_printf(MSG_DEBUG, "WPA: Dropping 3/4 4-Way handshake");
         			goto out;
 			}
		} else {
			/* 1/4 4-Way Handshake */
			...
			wpa_supplicant_process_1_of_4(sm, src_addr, key,
						      ver);
			if(eapol_drp){
				wpa_printf(MSG_DEBUG, "WPA: Dropping 1/4 4-Way handshake");
         			goto out;
 			}
		}
...
int wpa_install_cckm_newkeys(struct wpa_sm *sm, struct wpa_gtk_data *gd)
{
   /* Install PTK first */
	...
	if (wpa_sm_set_key(sm, alg, sm->bssid, 0, 1, key_rsc, rsclen,
  	 ...
  	 /* Install GTK */
	 gd->tx = 0;
	 wpa_supplicant_install_gtk(sm, gd, gd->key_rsc);
       }
       return 0;
}
#endif
