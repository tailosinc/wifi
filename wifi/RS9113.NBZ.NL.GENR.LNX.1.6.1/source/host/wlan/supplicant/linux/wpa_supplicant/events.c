...
#ifdef RSI_CHANGES
		/* Commenting below changes as channel status indication is 
...
//#ifdef CONFIG_DRIVER_RSI
		printf("ROAM : In %s and %d wpa_s->level %d \n", __func__, __LINE__, wpa_s->level);
	        if (wpa_s->conf->roam_threshold && (wpa_s->level < wpa_s->conf->roam_threshold)) {
...
//#endif
	...
	for (i = 0; i < wpa_s->last_scan_res_used; i++) {
		...
		*selected_ssid = wpa_scan_res_match(wpa_s, i, bss, group);
		...
		if(wpa_s->wpa_state != WPA_COMPLETED){
			...
			if(!found)
			{
				bss_prev = wpa_s->last_scan_res[i];
				index_bss = i;
				found++;
				continue;
			}
		...
		printf("In %s and %d \n", __func__, __LINE__);
		if (connected) {
			printf("ROAM : In %s and %d connectedAP_status %d wpa_s->level %d level %d \n", __func__, __LINE__, connectedAP_status, wpa_s->level, level);
	...
	{
		printf("<<<< In %s Line %d >>>\n",__func__, __LINE__);
		ie = wpa_scan_get_ie(bss_prev, WLAN_EID_SSID);
		ssid = ie ? ie + 2 : (u8 *) "";
		ssid_len = ie ? ie[1] : 0;
		wpa_dbg(wpa_s, MSG_DEBUG, "   selected BSS " MACSTR
			" ssid='%s'",
			MAC2STR(bss_prev->bssid), wpa_ssid_txt(ssid, ssid_len));
		printf("<<< Selecting ssid %s bssid "MACSTR" In %s Line %d len %d wpa_s %p >>>>\n", wpa_ssid_txt(ssid, ssid_len), MAC2STR(bss_prev->bssid), __func__, __LINE__, ssid_len, wpa_s);
		*selected_ssid = wpa_scan_res_match(wpa_s, index_bss, bss_prev, group);
		return wpa_bss_get(wpa_s, bss_prev->bssid, ssid, ssid_len);
			   ...
			   struct wpa_ssid *ssid)
{
	...
	if (wpa_s->reassociate ||
		...
		wpa_msg(wpa_s, MSG_ERROR, "Request association with " MACSTR,
				       ...
				       struct wpa_ssid *ssid)
{
	...
	printf("ROAM : In %s and %d \n", __func__, __LINE__);
	//wpa_supplicant_disassociate(wpa_s, 99);
#endif
	return 1;
...
#ifndef CONFIG_NO_STDOUT_DEBUG
{
	...
	case EVENT_CONNECT_FAILED_REASON:
		...
		break;
#ifdef RSI_CHANGES
	case EVENT_RSI_BGSCAN:
		wpa_printf(MSG_DEBUG,"BGSCAN EVENT RECEIVED ");
		wpa_supplicant_req_scan(wpa_s, 0, 0);
		break;
#ifdef RSI_CCX
