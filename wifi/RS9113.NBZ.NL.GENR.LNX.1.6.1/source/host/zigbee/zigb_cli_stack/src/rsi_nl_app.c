 ...
 * to receive queue.
 */
//void rsi_enqueue_to_rcv_q(rsi_pkt_t  *rcvPktPtr)
//{   //rsi_pkt_t  *rcvPktPtr;
    //cvPktPtr =(rsi_pkt_t *)Pkt;
//    printf("\nI am trying to enque packet i recv queue\n");
  //  rsi_enqueue_pkt(&rsi_linux_app_cb.rcv_queue, rsi_linux_app_cb.rcvPktPtr);
    

//    rsi_set_event(RSI_RX_EVENT);
//    printf("Enqueue happened and RX EVENT is SET");
...
//SIDDIQ
/*void rsi_enqueue_to_rcv_q(rsi_rx_pkt_t *Pkt)
{
...
/*void * RecvThreadBody(void * arg )
{
  ...
  while(1)
  {
    g_rcvnlink = (pkt_struct_t*)rsi_malloc(RSI_ZIGB_MAX_PAYLOAD_SIZE + RSI_RXPKT_HEAD_ROOM);
    if(g_rcvnlink == NULL)
    {
#ifdef RSI_DEBUG_PRINT
    ...
    g_rcvnlink->data = (uint8_t *)(((uint8_t *)g_rcvnlink) +  RSI_RXPKT_HEAD_ROOM);

    rsp_len = recv(rsi_linux_app_cb.nl_sd, g_rcvnlink->data , RSI_ZIGB_MAX_PAYLOAD_SIZE, 0);
    if(rsp_len < 0)
    {
      perror("recv");
#if ((defined LINUX_PLATFORM) && (defined RSI_DEBUG_PRINT))
    ...
    rsi_enqueue_to_rcv_q(g_rcvnlink);
...
void rsi_frame_print(uint8_t *buff, uint16_t bufLen)
{
