...
int16_t rsi_zigb_get_rssi(void)
{
	...
	getrssiFrameSnd rssi;

	pkt_size = sizeof(getrssiFrameSnd);

	rssi.pkt = 0;

	ret_val = rsi_zigb_execute_cmd((uint8_t  *)rsi_zigb_frameGetRssi,(uint8_t *)&rssi,pkt_size);
...
int16_t rsi_zigb_get_debug_info(uint8_t query_type,uint8_t data)
{
	...
	get_setDebugFrameSnd bufque;

	pkt_size = sizeof(get_setDebugFrameSnd);

	bufque.query_type = query_type;
	bufque.data = data;

	ret_val = rsi_zigb_execute_cmd((uint8_t *)rsi_zigb_frameDebug,(uint8_t *)&bufque,pkt_size);
...
void rsi_change_heart_beat_report(unsigned char flag)
{
 ...
 ret_val= rsi_zigb_execute_cmd((uint8_t *)rsi_zigb_frameHeartBeat,&state,1);
 ...
 *
 */
int16_t rsi_zigb_update_sas(APP_Startup_Attribute_Set_t *Startup)
{
	int16_t ret_val;

	ret_val = rsi_zigb_execute_cmd((uint8_t *)rsi_zigb_frameUpdateSAS, (uint8_t *)Startup,sizeof(APP_Startup_Attribute_Set_t));
